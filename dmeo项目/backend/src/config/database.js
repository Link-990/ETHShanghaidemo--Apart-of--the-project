...
async function setupTestDatabase() {
    try {
        ...
        if (!mongoUri) {
            logger.info('MongoDB URI not provided, skipping test database connection');
            return null;
        }

        // 将URI中的数据库名改为testdata
        const testMongoUri = mongoUri.replace(/\/[^\/]*(\?|$)/, '/testdata$1');
        ...
        // 连接到测试数据库（testdata）
        testConnection = await mongoose.createConnection(testMongoUri, options);
        
        logger.info('Test MongoDB (testdata) connected successfully');
        ...
        testConnection.on('disconnected', () => {
            logger.warn('Test MongoDB disconnected');
        });
