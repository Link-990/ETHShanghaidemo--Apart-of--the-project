...
class AIMonitoringService extends EventEmitter {
  constructor(config) {
    ...
    this.processedTransactions = new Set(); // 添加已处理交易的跟踪
  }
  ...
  shouldAnalyzeTransaction(tx) {
    ...
    if (this.processedTransactions.has(tx.hash)) {
      console.log(`交易 ${tx.hash} 已经处理过，跳过重复分析`);
      return false;
    }
  ...
  async performAIAnalysis() {
    ...
    for (const tx of transactions) {
      try {
        ...
        // 只在高风险时触发riskDetected事件，避免重复事件
        if (riskAnalysis.riskLevel >= this.riskThresholds.high) {
  ...
  async saveAnalysisResult(analysis) {
    try {
      ...
      if (existingRecord) {
        console.log(`⚠️ 交易 ${analysis.transactionHash} 已存在分析记录，跳过保存`);
