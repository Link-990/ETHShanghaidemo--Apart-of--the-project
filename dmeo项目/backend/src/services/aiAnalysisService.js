...
class AIAnalysisService {
    constructor() {
        ...
        // Web3 初始化 - 支持私链连接
        try {
            const web3ProviderUrl = process.env.WEB3_PROVIDER_URL || 'http://localhost:8545';
            const chainId = process.env.CHAIN_ID || '1337';
            
            this.web3 = new Web3(web3ProviderUrl);
            
            // 私链连接配置
            this.privateChainConfig = {
                url: web3ProviderUrl,
                chainId: chainId,
                networkId: process.env.NETWORK_ID || chainId
            };
            
            logger.info(`Web3 client initialized successfully for private chain: ${web3ProviderUrl}`);
            logger.info(`Chain ID: ${chainId}, Network ID: ${this.privateChainConfig.networkId}`);
            
            // 测试私链连接
            this.testPrivateChainConnection();
            
        } catch (error) {
            logger.error('Failed to initialize Web3 for private chain:', error);
            this.web3 = null;
        }
    ...
    // 测试私链连接
    async testPrivateChainConnection() {
        try {
            if (!this.web3) {
            ...
            logger.info('🔗 Testing private chain connection...');
            ...
            logger.info(`✅ Private chain connection successful:`);
            ...
            // 验证链ID是否匹配配置
            if (chainId.toString() !== this.privateChainConfig.chainId) {
                logger.warn(`⚠️ Chain ID mismatch: expected ${this.privateChainConfig.chainId}, got ${chainId}`);
            }
            
            return true;
            
        } catch (error) {
            logger.error('❌ Private chain connection test failed:', error.message);
            return false;
        }
    }
    ...
    async buildComprehensiveAuditPrompt(contractAddress, contractInfo, transactionHistory, transactionData) {
        // 获取私链网络信息
        const privateChainInfo = this.getPrivateChainContext();
        
        // 获取当前区块信息
        const currentBlock = await this.getPrivateChainCurrentBlock();
        privateChainInfo.currentBlock = currentBlock;
        
        return `
        🔍 **ETH+AI融合系统深度分析请求**
        
        **私链网络环境:**
        - 网络URL: ${privateChainInfo.url}
        - 链ID: ${privateChainInfo.chainId}
        - 网络ID: ${privateChainInfo.networkId}
        - 当前区块: ${privateChainInfo.currentBlock || 'N/A'}
    ...
    // 获取私链网络上下文信息
    getPrivateChainContext() {
        return {
            url: this.privateChainConfig?.url || 'http://localhost:8545',
            chainId: this.privateChainConfig?.chainId || '1337',
            networkId: this.privateChainConfig?.networkId || '1337',
            currentBlock: null // 将在实际调用时获取
        };
    }

    // 获取私链当前区块信息
    async getPrivateChainCurrentBlock() {
        try {
            if (!this.web3) {
                return null;
            }
            ...
            logger.error('Failed to get current block from private chain:', error);
            return null;
        }
    }
