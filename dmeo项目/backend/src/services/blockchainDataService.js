...
class BlockchainDataService extends EventEmitter {
  ...
  async initialize() {
    try {
      this.web3 = new Web3(this.config.privateChain.url);
      ...
      console.log(`已连接到私链: ${this.config.privateChain.url}`);
      this.emit('connected', { url: this.config.privateChain.url });
      
      return true;
    } catch (error) {
      console.error('连接私链失败:', error);
      this.isConnected = false;
      throw error;
    }
  }
